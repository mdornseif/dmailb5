...
int main(int argc, char *argv[])
{
  ...
  netstr_read_buf(buffer_0, &tmp);
  switch *tmp.s
    {
    case 'P':
      rijndaelDncrypt_cbc(tmp.s, tmp.len, &out);
      s = out.s;
      l = netstr_getlen(s);
      stralloc_copys(&tmp, "");
      stralloc_readyplus(&tmp, 1024*1024);
      tmp.len += 1024*1024;
      dmb_decompress(tmp.s, &tmp.len, s, l);
      
    default:
      buffer_puts(buffer_2, "unknown type");
